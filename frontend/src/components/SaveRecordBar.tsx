...
export function SaveRecordBar({
  ...
  const resolvedButtonLabel =
    buttonLabel ?? (batchMode ? "完成当前条目并进入下一条" : "记录处理过程与满意版本");
  ...
  return (
    <div className="flex flex-col gap-4 rounded-panel border border-white/10 bg-ink p-5 text-paper shadow-[0_24px_60px_rgba(29,41,54,0.28)] md:flex-row md:items-center md:justify-between">
      <div className="max-w-2xl">
        ...
        <p className="mt-2 text-sm leading-7 text-white/82">{resolvedDescription}</p>
        {statusText ? <p className="mt-2 text-sm text-white/68">{statusText}</p> : null}
      </div>
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        >
          {isSaving ? "保存中..." : resolvedButtonLabel}
        </button>
      </div>
    </div>
  );
