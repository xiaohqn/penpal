 ...
 * - annotations / onAddAnnotation / onRemoveAnnotation：可选的片段批注数据与操作回调。
 * - eyebrow / title / placeholder：可选的文案覆盖项，用于在不同业务场景下复用同一套编辑器。
 * - showAnnotations：控制是否显示“AI 回复片段批注”区域。
 * - sidePanel：可选的并排侧栏内容，用于把评价或补充信息固定在编辑区旁边。
 * 输出：
 * - 渲染一个可编辑的大文本润色区域，并在需要时附带片段批注能力。
 * 作用：
    ...
    <section className="rounded-[28px] border border-line bg-white/78 p-6 shadow-soft">
      <div className="mb-4">
        ...
        <p className="mt-2 text-sm leading-7 text-ink/66">
          在保留当前风格标签的前提下，把草稿修成能直接发送的最终版本。你也可以直接在 AI 回复里划词做片段批注，再基于这些说明出新版本。
        </p>
      </div>
      ...
      {showAnnotations ? (
        <section className="mt-5 rounded-[26px] border border-line bg-paper/68 p-4">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm uppercase tracking-[0.2em] text-amber">AI 回复片段批注</p>
          </div>
          <p className="mt-2 text-sm leading-7 text-ink/66">
          ...
          {selectionRange ? (
            <div className="mt-4 rounded-[22px] border border-amber/35 bg-white/82 p-4">
              <p className="text-xs uppercase tracking-[0.16em] text-ink/42">当前选中片段</p>
          ...
          <details className="mt-4 rounded-[22px] border border-line bg-white/82 p-4">
            <summary className="cursor-pointer list-none">
              <div className="flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
                <p className="text-xs uppercase tracking-[0.16em] text-ink/42">片段高亮预览</p>
            ...
            <div
              ...
              dangerouslySetInnerHTML={{ __html: highlightedPreview || "暂无片段批注。" }}
            />
          </details>
              ...
              {annotations.map((annotation, index) => (
                <div key={annotation.id} className="rounded-[22px] border border-line bg-white/82 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-xs uppercase tracking-[0.16em] text-ink/42">片段批注 {index + 1}</p>
