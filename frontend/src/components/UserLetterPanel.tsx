    ...
    <section className="rounded-panel border border-line bg-white/86 p-4 shadow-soft backdrop-blur md:p-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex min-w-0 flex-col gap-2 md:flex-row md:items-center">
          <h2 className="font-serif text-2xl text-ink md:text-3xl">原始来信</h2>
          {batchMeta ? (
      ...
      <textarea
        ...
        readOnly={readOnly}
        rows={rows}
        className="mt-3 w-full resize-y rounded-[20px] border border-transparent bg-paper/75 px-4 py-3 text-[15px] leading-7 text-ink shadow-[inset_0_1px_0_rgba(255,255,255,0.75)] outline-none transition focus:border-amber focus:bg-white focus:shadow-[0_0_0_4px_rgba(79,110,140,0.14)]"
      />
    </section>
  );
}
